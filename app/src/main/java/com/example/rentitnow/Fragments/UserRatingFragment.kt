...
class UserRatingFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        if(arguments != null) {
            ...
            userRating = requireArguments().getSerializable(USER_RATING) as UserRating
            if(userRating != null) {
        ...
        submit_Button.setOnClickListener {
            ...
            val userRating = UserRating(booking.userId, booking.vendorId, bookingId, rating, feedback)
            database.getReference("UserRatings").child(bookingId).setValue(userRating).addOnSuccessListener {
                (context as NavigationActivityUser).supportFragmentManager.beginTransaction()
                    .replace(R.id.fragment_container_user, PostFeedbackFragment(), "findThisFragment")
                    .addToBackStack(null)
                    .commit()
            }
